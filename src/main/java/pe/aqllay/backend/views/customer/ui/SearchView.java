...
public class SearchView extends Panel {
    ...
    private CustomerWindow app;

    public SearchView(final CustomerWindow app) {
        this.app = app;
        addStyleName("view");
        ...
        for (int i = 0; i < CustomerContainer.NATURAL_COL_ORDER.length; i++) {
            fieldToSearch.addItem(CustomerContainer.NATURAL_COL_ORDER[i]);
            fieldToSearch.setItemCaption(CustomerContainer.NATURAL_COL_ORDER[i],
                    CustomerContainer.COL_HEADERS_ENGLISH[i]);
        }
